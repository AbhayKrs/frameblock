            ...
            {templates.length > 0 && templates.map(item => (
                <SwiperSlide key={item._id}>
                    <div className='relative'>
                        ...
                        <div id="active-text" class="absolute flex flex-col space-y-1 inset-0 m-auto h-fit flex-row items-center p-2 w-full bg-slate-300 dark:bg-neutral-800">
                            ...
                            <button onClick={() => handleClick(item._id)} className='font-caviar text-sm py-1 px-3 rounded-lg bg-gray-700 dark:bg-gray-100'>{btnTitle}</button>
                        </div>
                    </div>
                </SwiperSlide>
            ))}
